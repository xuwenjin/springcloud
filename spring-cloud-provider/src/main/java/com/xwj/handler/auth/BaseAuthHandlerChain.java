...
//				return ApiResponseData.unauthorizedError("请求过期");
			...
			if (MapUtils.isEmpty(account)) {
				loginInfoService.setUserLog(phone, accessToken, userIp, path, "logout", "无效的accessToken");
			...
			if (lastLoginInfo == null || StringUtils.isEmpty(lastAccessToken)) {
				...
				loginInfoService.setUserLog(phone, accessToken, userIp, path, "logout", "用户未登陆");
				loginInfoService.removeLoginInfo(accessToken);
			...
			if (!StringUtils.equals(lastAccessToken, accessToken)) {
				loginInfoService.setUserLog(phone, accessToken, userIp, path, "logout", "该用户已在其他APP登录，用户被下线!");
				loginInfoService.removeLoginInfo(accessToken);
